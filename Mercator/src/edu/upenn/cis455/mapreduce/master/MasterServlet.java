...
public class MasterServlet extends HttpServlet {
	...
	// initialization
	public void init () {
		System.out.println("[DEBUG] Initializing Master Servlet");
		/*
		WorkerStatusParams worker = new WorkerStatusParams();
		worker.port = 8081;
		worker.status = "idle";
		worker.timeRecieved = System.currentTimeMillis();
		this.mapworkerStatusParams.put("localhost:8081",worker);
		worker.port = 8082;
		worker.status = "idle";
		worker.timeRecieved = System.currentTimeMillis();
		this.mapworkerStatusParams.put("localhost:8082",worker);
		worker.port = 8083;
		worker.status = "idle";
		worker.timeRecieved = System.currentTimeMillis();
		this.mapworkerStatusParams.put("localhost:8083",worker);
		 */
	}
	...
	public void storeWorkerStatus(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
		...
		// System.out.println("Worker status: "+ this.mapworkerStatusParams.keySet().size());
		out.close();
	...
	public void displayStatusRequest(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
		...
		out.println("<table align = 'center'><tr><th>Active Worker</th><th>Status</th></tr>");
		...
		if(this.isIdle){
			...
			out.println("<form name='map' method = 'POST'>");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
		...
		for(String workers: this.mapworkerStatusParams.keySet()) {
			...
			// System.out.println("[DEBUG] Inside run crawler + " + requestUrl);
			for (String subseed : this.mapWorkerSeedURL.get("worker"+iWorker))
			{
				String requestParams = "";
				int iSubWorker = 1;
				int index = 1;
				...
				requestParams += "&" + "numWorkers=" + this.mapworkerStatusParams.keySet().size();
				requestParams += "&" + "seed=" + subseed;
				System.out.println("[SUBSEED MASTER] Enqueue + " + subseed);
				// send request and request params to MasterClient 
				MasterClient clientThread = new MasterClient (requestUrl, requestParams);
				clientThread.start();
			}
